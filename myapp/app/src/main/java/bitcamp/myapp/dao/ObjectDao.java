...
public abstract class ObjectDao {
  ...
  public void insert(Object value) {
    ...
    //리스트의 마지막 노드와  연결한다.
    // 만약 리스트의 노드가 없으면 현재 노드를 시작 노드 겸 마지막 노드로 설정한다.
    //size == 0 , head == null 같다
    if (this.tail == null) {
      this.head = this.tail = node;
    }else {

      // => 마지막 노드의 next 필드에 새노드 주소를 담는다.
      this.tail.next = node;

      //=> 새 노드가 마지막 노드가 되도록 tail의 주소를 바꾼다.
      this.tail = node;
    }
    //한번 등록 할때마다 크기를 개수를 카운트 한다 .
    this.size++;
  }
  public Object[] findAll() {

    //리스트에 보관된 값을 담을 배열을 준비한다.
  ...
  public void delete(Object value) {
    ...
    if (index == 0) {
      //만약 head가 가리키는 다음 노드를 시작 노드로 설정한다.
      head = head.next;
    ...
    while(i < index) {
      //커서를 다음 노드로 이동하기 전에 이전 노드의 주소를 보관한다.
      prevNode = cursor;
