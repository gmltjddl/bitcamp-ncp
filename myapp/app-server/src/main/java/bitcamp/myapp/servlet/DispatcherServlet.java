...
public class DispatcherServlet extends HttpServlet {
  ...
  public void service(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      RequestHandlerMapping requsetHandlerMapping= (RequestHandlerMapping) ctx.getAttribute(pathInfo);
      if (requsetHandlerMapping == null) {
        request.getRequestDispatcher("/NotFoundController.jsp").forward(request, response);
        return;
      }

      // 페이지 컨트롤러 실행
      Object[] arguments = prepareRequesetHandlerArguments(requsetHandlerMapping.method,request,response);

      String view =(String) requsetHandlerMapping.method.invoke(
          requsetHandlerMapping.controller, arguments); // controller.execute(request,response)
  ...
  private Object[] prepareRequesetHandlerArguments(
      Method method,
    ...
    for(int i=0; i<params.length; i++) {
      ...
      if(paramType== HttpSession.class){
        ...
        RequsetParam anno = params[i].getAnnotation(RequsetParam.class);
        if(anno !=null) {
          ...
          } else if (paramType ==char.class || paramType == char.class) {
            arguments[i] =Character.(paramValue);
